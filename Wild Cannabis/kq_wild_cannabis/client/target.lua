...
RegisterNetEvent('kq_wild_cannabis:target:bag')
AddEventHandler('kq_wild_cannabis:target:bag', function(data)
    CraftItem(data.key)
end)


RegisterNetEvent('kq_wild_cannabis:target:sell')
AddEventHandler('kq_wild_cannabis:target:sell', function(data)
    SellItem(data.key)
end)
