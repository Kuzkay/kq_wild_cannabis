...
Citizen.CreateThread(function()
    ...
    for k, crafting in pairs(Config.crafting) do
        ...
        AddEntityToTargeting(prop, 'crafting_' .. prop, L('Bag the cannabis'), 'kq_wild_cannabis:target:bag', k)
    
        if crafting.showBlip then
            CreateBlip(vector3(loc.x, loc.y, loc.z), 408, 52, 255, 1.0, L('Cannabis packaging workbench'))
        end
    end
    ...
    for k, selling in pairs(Config.selling) do
        ...
        AddEntityToTargeting(ped, 'selling_' .. ped, L('Sell the cannabis'), 'kq_wild_cannabis:target:sell', k)
        
        if selling.showBlip then
            CreateBlip(vector3(loc.x, loc.y, loc.z), 408, 52, 255, 1.0, L('Wild cannabis buyer'))
        end
    end
end)
        ...
        while true do
            ...
            for k, crafting in pairs(Config.crafting) do
                ...
                if distance < 20 then
                    sleep = 1000
                    if distance < 2 then
                        Draw3DText(loc.x, loc.y, loc.z - 0.5, L('Press ~g~[~w~{KEYBIND}~g~]~w~ to bag the cannabis'):gsub('{KEYBIND}', Config.keybinds.interact.label), 4, 0.035, 0.035)
        ...
        while true do
            ...
            for k, selling in pairs(Config.selling) do
                ...
                if distance < 20 then
                    sleep = 1000
                    if distance < 2 then
                        Draw3DText(loc.x, loc.y, loc.z, L('Press ~g~[~w~{KEYBIND}~g~]~w~ to sell the cannabis'):gsub('{KEYBIND}', Config.keybinds.interact.label), 4, 0.035, 0.035)
...
function CraftItem(craftingKey)
    ...
    TriggerServerEvent('kq_wild_cannabis:server:craft', craftingKey)
end
...
function SellItem(sellingKey)
    ...
    TriggerServerEvent('kq_wild_cannabis:server:sell', sellingKey)
end

RegisterNetEvent('kq_wild_cannabis:client:notify')
AddEventHandler('kq_wild_cannabis:client:notify', function(message)
    ShowTooltip(message)
end)
